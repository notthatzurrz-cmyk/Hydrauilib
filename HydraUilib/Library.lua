...
-- Clean up any existing ScreenGui or BlurEffect from a previous execution
if getgenv and getgenv().Library and type(getgenv().Library.Unload) == "function" then
    pcall(function() getgenv().Library:Unload() end)
end
pcall(function()
    for _, child in ipairs(CoreGui:GetChildren()) do
        if child:IsA("ScreenGui") and (child.Name == "ScreenGui" or child.Name == "LinoriaGui" or child.Name == "HydraGui") then
            if child:FindFirstChild("WatermarkOuter") or child:FindFirstChild("Window") or child:FindFirstChild("NotificationArea") then
                child:Destroy()
            end
        end
    end
    for _, b in ipairs(Lighting:GetChildren()) do
        if b:IsA("BlurEffect") and b.Name == "LinoriaBlur" then
            b:Destroy()
        end
    end
end)
...
local Library = {
    ...
    FontColor = Color3.fromRGB(240, 242, 245);
    MainColor = Color3.fromRGB(14, 15, 18);
    BackgroundColor = Color3.fromRGB(8, 9, 11);
    AccentColor = Color3.fromRGB(230, 232, 238);
    OutlineColor = Color3.fromRGB(30, 32, 38);
    RiskColor = Color3.fromRGB(235, 75, 75),
...
function Library:ResumeLayout()
    if Library._ResumingLayout then
        return
    end
    Library._ResumingLayout = true
    Library.LayoutSuspended = false

    local dirty = Library.DirtyGroupboxes
    Library.DirtyGroupboxes = {}
    for groupbox in pairs(dirty) do
    ...
    if win and win.Tabs then
        ...
        for _, tab in pairs(win.Tabs) do
            if type(tab) == 'table' and not seenTabs[tab] then
                seenTabs[tab] = true
...
-- Applies a control's default/saved state at build time. Isolated (and loud) so a failing
-- optional feature cannot abort the entire UI construction while still being debuggable.
function Library:FireInitialCallback(Idx, Callback, Value)
    local ok, err = pcall(Library.SafeCallback, Library, Callback, Value);
    if (not ok) and err ~= nil then
        warn(("[Hydra] Initial state for '%s' failed to apply: %s"):format(tostring(Idx), tostring(err)));
    end;
end;
...
do
    ...
    function Funcs:AddRow(Columns)
        ...
        for i = 1, ColumnsCount do
            ...
            function Box:Resize()
                if Library.LayoutSuspended or isResizing then return end
                isResizing = true
                local maxHeight = 0
                ...
                if math.abs(RowOuter.Size.Y.Offset - maxHeight) > 1 then
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        -- Immediately fire default callback so runtime features apply on startup
        if Info.Default ~= nil and Info.Default ~= '' and not Info.Finished then
            Library:FireInitialCallback(Idx, Textbox.Callback, Textbox.Value);
        end
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        if Info.Default ~= nil then
            Library:FireInitialCallback(Idx, Toggle.Callback, Toggle.Value);
        end
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        -- Immediately fire default callback so runtime features apply on startup
        if Info.Default ~= nil then
            Library:FireInitialCallback(Idx, Slider.Callback, Slider.Value);
        end
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        -- Immediately fire default callback so runtime features apply on startup
        if next(Defaults) then
            Library:FireInitialCallback(Idx, Dropdown.Callback, Dropdown.Value);
        end
    ...
    function Funcs:AddDependencyBox()
        ...
        function Depbox:Resize()
            if Library.LayoutSuspended or isResizing then
                return
            end
            isResizing = true
            ...
            if math.abs(Holder.Size.Y.Offset - depH) > 1 then
...
function Library:CreateWindow(...)
    ...
    function Window:AddTab(Name)
        ...
        local function RecalculateTabs()
            local tabCount = 0
            for _ in next, Window.Tabs do tabCount = tabCount + 1 end
            tabCount = tabCount + 1
            if tabCount > 0 then
                local totalWidth = TabArea.AbsoluteSize.X > 0 and TabArea.AbsoluteSize.X or 500
                local padding = Config.TabPadding or 4
                local available = totalWidth - (padding * (tabCount - 1))
                local perTab = math.max(48, math.floor(available / tabCount))
                for _, otherTab in next, Window.Tabs do
                    if otherTab.TabButton then
                        otherTab.TabButton.Size = UDim2.new(0, perTab, 1, 0)
                    end
                end
                TabButton.Size = UDim2.new(0, perTab, 1, 0)
            end
        end
        pcall(RecalculateTabs)
        ...
        local leftLayout = LeftSide:FindFirstChildOfClass('UIListLayout')
        if leftLayout then
            leftLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
                local newY = leftLayout.AbsoluteContentSize.Y + 16
                if math.abs(LeftSide.CanvasSize.Y.Offset - newY) > 1 then
                    LeftSide.CanvasSize = UDim2.fromOffset(0, newY);
                end
            end)
        end
        local rightLayout = RightSide:FindFirstChildOfClass('UIListLayout')
        if rightLayout then
            rightLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
                local newY = rightLayout.AbsoluteContentSize.Y + 16
                if math.abs(RightSide.CanvasSize.Y.Offset - newY) > 1 then
                    RightSide.CanvasSize = UDim2.fromOffset(0, newY);
                end
            end)
        end
        ...
        function Tab:AddGroupbox(Info)
            ...
            function Groupbox:Resize()
                ...
                isResizing = true
                local layoutH = layout and layout.AbsoluteContentSize.Y or 0
                if layoutH <= 0 then
                    local Size = 0
                ...
                local targetH = 20 + layoutH + 6
                if math.abs(BoxOuter.Size.Y.Offset - targetH) > 1 then
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name)
                ...
                function Tab:Resize()
                    ...
                    local targetH = headerH + layoutH + 6
                    if math.abs(BoxOuter.Size.Y.Offset - targetH) > 1 then
                        BoxOuter.Size = UDim2.new(1, 0, 0, targetH);
                    end
                    isResizing = false
                ...
                if layout then
                    layout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
                        if Container.Visible or Tabbox.SelectedTab == Tab then
                            Tab:Resize()
                        end
                    end)
                end
        ...
        TabButton.MouseEnter:Connect(function()
            if Window.ActiveTab ~= Tab then
                TabButton.BackgroundColor3 = Color3.fromRGB(18, 20, 24)
            end
        end)
    ...
    function Library:Toggle()
        ...
        if Library.Toggled then
            ...
            if Library.ShowCustomCursor and type(Drawing) == 'table' and type(Drawing.new) == 'function' then
                task.spawn(function()
                    local State = InputService.MouseIconEnabled;
                    local okC, Cursor = pcall(Drawing.new, 'Triangle');
                    local okO, CursorOutline = pcall(Drawing.new, 'Triangle');
                    if not okC or not okO or not Cursor or not CursorOutline then
                        if Cursor and Cursor.Remove then pcall(function() Cursor:Remove() end) end
                        if CursorOutline and CursorOutline.Remove then pcall(function() CursorOutline:Remove() end) end
                        return
                    end
    ...
    Window.TabBarInner = TabBarInner;
    Window.TabBarOuter = TabBarOuter;
...
Library.BackgroundImageURL = nil
Library.BackgroundImageTransparency = 0.15
Library.BackgroundImageObject = nil

function Library:SetBackgroundTransparency(transparency)
    Library.BackgroundImageTransparency = tonumber(transparency) or 0.15
    if Library.BackgroundImageObject then
        Library.BackgroundImageObject.ImageTransparency = Library.BackgroundImageTransparency
    end
end

local _bgLoadingThread = nil
function Library:SetBackgroundImage(urlOrAsset)
    local win = Library.Window
    if not win then return end
    local inner = win.Inner or (win.Holder and win.Holder:FindFirstChild("Inner")) or win.Holder
    if not inner then return end

    if not urlOrAsset or urlOrAsset == "" then
        Library.BackgroundImageURL = ""
        if Library.BackgroundImageObject then
            Library.BackgroundImageObject.Visible = false
        end
        -- Restore default panel opacities
        pcall(function()
            if win.Inner then win.Inner.BackgroundTransparency = 0 end
            if win.MainSectionOuter then win.MainSectionOuter.BackgroundTransparency = 0 end
            if win.MainSectionInner then win.MainSectionInner.BackgroundTransparency = 0 end
            if win.TabBarOuter then win.TabBarOuter.BackgroundTransparency = 0 end
            if win.TabBarInner then win.TabBarInner.BackgroundTransparency = 0 end
            if win.TabContainer then win.TabContainer.BackgroundTransparency = 0 end
        end)
        return
    end

    local urlStr = tostring(urlOrAsset):gsub("^%s+", ""):gsub("%s+$", "")
    if urlStr == "" then return end

    -- Cancel any existing background loading thread
    if _bgLoadingThread then
        task.cancel(_bgLoadingThread)
        _bgLoadingThread = nil
    end

    _bgLoadingThread = task.spawn(function()
        local assetId = nil
        if type(urlOrAsset) == "number" or urlStr:match("^%d+$") then
            assetId = "rbxassetid://" .. tostring(urlStr)
        elseif urlStr:match("^rbxassetid://") or urlStr:match("^rbxthumb://") or urlStr:match("^rbxasset://") then
            assetId = urlStr
        elseif urlStr:match("^https?://[%w-_%.%?%.:/%+=&]+") then
            local getAsset = getcustomasset or getsynasset
            if getAsset and writefile and readfile then
                local success, resId = pcall(function()
                    local h = 5381
                    for i = 1, #urlStr do
                        h = bit32.band(bit32.lshift(h, 5) + h + string.byte(urlStr, i), 0xFFFFFFFF)
                    end
                    local fileName = "meowhook_bg_" .. string.format("%08x", h) .. ".png"
                    if isfile and isfile(fileName) then
                        return getAsset(fileName)
                    end
                    local res = game:HttpGet(urlStr)
            ...
            -- Not a recognizable format/URL yet
            return
        end

        if not assetId or assetId == "" then return end
        Library.BackgroundImageURL = urlStr

        if not Library.BackgroundImageObject or not Library.BackgroundImageObject.Parent then
            local bgImg = Instance.new("ImageLabel")
            bgImg.Name = "UIBackgroundImage"
            ...
            bgImg.Parent = inner
            Library.BackgroundImageObject = bgImg
        end

        Library.BackgroundImageObject.Image = assetId
        Library.BackgroundImageObject.ImageTransparency = Library.BackgroundImageTransparency or 0.15
        Library.BackgroundImageObject.Visible = true

        -- Adjust panel translucency so the background is cleanly visible behind them
        pcall(function()
            if win.Inner then win.Inner.BackgroundTransparency = 0.25 end
            if win.MainSectionOuter then win.MainSectionOuter.BackgroundTransparency = 0.35 end
            if win.MainSectionInner then win.MainSectionInner.BackgroundTransparency = 0.35 end
            if win.TabBarOuter then win.TabBarOuter.BackgroundTransparency = 0.30 end
            if win.TabBarInner then win.TabBarInner.BackgroundTransparency = 0.30 end
            if win.TabContainer then win.TabContainer.BackgroundTransparency = 0.35 end
        end)
    end)
end
