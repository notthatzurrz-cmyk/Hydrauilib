...
do
    ...
    function Funcs:AddDependencyBox()
        ...
        function Depbox:Resize()
            ...
            local newH = UDim2.new(1, 0, 0, depH);
            if Holder.Size ~= newH then
                Holder.Size = newH;
                Groupbox:Resize();
            end;
        end;
    ...
    function Window:AddTab(Name)
        ...
        if leftLayout then
            leftLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
                LeftSide.CanvasSize = UDim2.fromOffset(0, leftLayout.AbsoluteContentSize.Y + 16);
            end)
        end
        local rightLayout = RightSide:FindFirstChildOfClass('UIListLayout')
        if rightLayout then
            rightLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
                RightSide.CanvasSize = UDim2.fromOffset(0, rightLayout.AbsoluteContentSize.Y + 16);
            end)
        end
        ...
        function Tab:AddGroupbox(Info)
            ...
            function Groupbox:Resize()
                ...
                BoxOuter.Size = UDim2.new(1, 0, 0, 20 + layoutH + 6)
            end
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(Name)
                ...
                function Tab:Resize()
                    ...
                    BoxOuter.Size = UDim2.new(1, 0, 0, headerH + layoutH + 6);
                end;
...
function Library:SetBackgroundImage(urlOrAsset)
    ...
    local assetId = nil
    -- Direct asset ID or number
    if type(urlOrAsset) == "number" or urlOrAsset:match("^%d+$") then
        assetId = "rbxassetid://" .. tostring(urlOrAsset)
    elseif urlOrAsset:match("^rbxassetid://") or urlOrAsset:match("^rbxthumb://") or urlOrAsset:match("^rbxasset://") then
        assetId = urlOrAsset
    elseif urlOrAsset:match("^https?://") then
        -- Web URL: download and convert using getcustomasset if available
        local getAsset = getcustomasset or getsynasset
        if getAsset and writefile and readfile then
            local success, hashOrContent = pcall(function()
                local fileName = "meowhook_bg_" .. string.format("%08x", (function()
                    local h = 5381
                    for i = 1, #urlOrAsset do
                        h = bit32.band(bit32.lshift(h, 5) + h + string.byte(urlOrAsset, i), 0xFFFFFFFF)
                    end
                    return h
                end)()) .. ".png"
            ...
            if success and hashOrContent then
                assetId = hashOrContent
            end
        end
        if not assetId then
            -- Fallback if executor does not require local file mapping
            assetId = urlOrAsset
        end
    end

    if not assetId then return end
    Library.BackgroundImageURL = urlOrAsset
    ...
    -- Adjust panel translucency so the background is visible through gaps and behind panels
    pcall(function()
